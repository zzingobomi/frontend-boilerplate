...
describe("User Profile", () => {
  ...
  it("Open user profile and check data is displayed", () => {
    ...
    cy.getBySel("user-name").should("contain.text", userName);
    ...
    cy.get('[data-testid="user-name"] input').should("contain.value", userName);
  });

  it("Update user data", () => {
    cy.intercept("PATCH", "api/v1/auth/me").as("profileUpdate");
    cy.visit("/profile/edit");
    cy.getBySel("user-name").type(`{selectAll}Oscar`);
    cy.get('[data-testid="user-name"] input').should("contain.value", "Oscar");
    ...
    cy.getBySel("user-profile").click();
    cy.location("pathname").should("include", "/profile");
    cy.getBySel("user-name").should("contain.text", "Oscar");
    cy.getBySel("user-email").should("contain.text", email);
    cy.getBySel("edit-profile").click();
    cy.location("pathname").should("include", "/profile/edit");
  ...
  it("Want to leave page", () => {
    ...
    cy.location("pathname").should("include", "/profile");
    cy.getBySel("user-name").should("be.visible").and("contain.text", userName);
